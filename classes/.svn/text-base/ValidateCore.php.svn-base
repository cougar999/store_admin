...
class ValidateCore
{
	...
	{
		if (!$url || $url == 'http://')
			$errors[] = displayError('Please specify module URL');
		elseif (substr($url, -4) != '.tar' && substr($url, -4) != '.zip' && substr($url, -4) != '.tgz' && substr($url, -7) != '.tar.gz')
			$errors[] = displayError('Unknown archive type');
		else
		{
			if ((strpos($url, 'http')) === false)
				$url = 'http://'.$url;
			if (!is_array(@get_headers($url)))
				$errors[] = displayError('Invalid URL');
		}
		if (!count($errors))
			return true;
		return false;
	...
	*/
	...
	public static function isUpdateCache($detail){
		if($detail == '000000')
			return true;
		else 
			return false;
